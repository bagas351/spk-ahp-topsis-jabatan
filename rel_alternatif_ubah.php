...
<div class="row">
    <div class="col-sm-4">
        ...
        <form method="post">
            <?php
            $rows = $db->get_results("SELECT ra.ID, k.kode_kriteria, k.nama_kriteria, ra.nilai FROM tb_rel_alternatif ra INNER JOIN tb_kriteria k ON k.kode_kriteria=ra.kode_kriteria  WHERE kode_alternatif='" . get('ID') . "' ORDER BY kode_kriteria");
            foreach ($rows as $row) : ?>
